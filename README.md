...
- **模擬審査会(本番形式)** — 実物のC級審査会筆記試験と**同じ構成(問1〜問7)・同じ出題数(66項目)・同じ配点(100点満点)**で、1枚の答案を最後まで解いてから採点します。コート図の名称と寸法、施設用具の数値、セカンドレフェリーの判定、反則とハンドシグナルの対応、ラインアップシートを使ったポジションの反則、○×など、本番と同じ形式で出題されます。**問題文・図はすべてこのアプリの自作**で、特定の団体が作成した練習問題の複製ではありません。実物の練習問題に合格点の記載がないため、合否判定は行わず得点と問別の内訳のみを表示します。
...
├── exam.json                   # 模擬審査会の問題データ(自動生成)
├── exam_figures.json           # 模擬審査会の図データ(自動生成)
