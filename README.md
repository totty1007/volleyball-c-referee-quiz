...
**C級審判員資格(6人制競技規則・一般共通)の筆記試験対策**用Webアプリです。演習問題・
模擬試験・苦手問題の復習ができます。ビルド不要の静的サイトなので、**GitHub Pages で
無料公開**できます。

> ⚠️ 本アプリは学習用の非公式教材です。出題内容は(公財)日本バレーボール協会
> 「6人制バレーボール競技規則」の一般6人制ルールを基準に作成しています。
> 実際のC級審判員資格取得審査会の筆記試験は、小学生バレーボール(付録2)の特例
> ではなく**一般6人制の標準ルールで出題される**ことを、相模原バレーボール協会・
> 神奈川県バレーボール協会が実際に配布した練習問題(2026-08-27時点)で確認して
> 作成しています。ただし**規則は年度により改定されます**。受験前には必ず最新の
> 公式ルールブック・受験地域(都道府県・市区町村協会)の講習会資料で確認してください。
>
> なお、小学生の試合を実際に主催・運営する際は、付録2の特別ルール(コート
> 16m×8m・ネット高さ2.00m・ボール200～220g・21点制・リベロ制度なし・選手交代
> 12回など)が適用されます。これは筆記試験の出題対象ではなく、あくまで実際の
> 試合運営との違いとして参考情報です。
...
- **演習モード** — カテゴリ(コート・用具／チーム編成／ローテーション・ポジション／プレー・反則／リベロ制度／記録用紙の書き方／得点・セット・タイムアウト／審判員の役割・シグナル／大会運営)を選んで1問ずつ解説つきで学習できます。
