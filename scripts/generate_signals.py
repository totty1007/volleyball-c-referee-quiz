...
def net_edge(x, y_band, y_floor, w=9):
    """真正面から見たネット。視線がネットの面と重なるので網目は見えず、
    上端(白帯)から床まで伸びる1本の太い縦線になる。教科書の㉑オーバーネットが
    この描き方をしている。
    この線より右にある手は「ネットの向こう側(相手コート側)にある手」、
    線の上端より上にある手は「白帯より上にある手」と読める。"""
    return (f'<line x1="{x}" y1="{y_band}" x2="{x}" y2="{y_floor}" stroke="{HALO}" '
            f'stroke-width="{w + 6}"/>'
            f'<line x1="{x}" y1="{y_band}" x2="{x}" y2="{y_floor}" stroke="{INK}" '
            f'stroke-width="{w}"/>')
...
# v3.7までは㉑オーバーネットだけ svg_wrap_two()(横から見た図と正面から見た図の
# 2面図)を使っていたが、教科書の図が1枚で描き切っているのに合わせて1面図に
# 直したので、v3.8で削除した。
...
def add_raw(id_, name, svg, hint="", wide=False):
    ...
    wide=True は横長の図の印。app.js/style.css がこの印を見て図の枠を広くする
    (横長の図を他と同じ幅に押し込むと小さすぎて読めない)。v3.8で㉑の2面図を
    1面図に直したため、現在この印を使っているシグナルは無い。"""
...
# 教科書の図(2026-08-28にいただいた写真)と同じ1枚図にする。構図は次のとおり。
#   ・審判は正面向き。視線がネットの面と重なるので、ネットは網目の見えない
#     1本の太い縦線になる(net_edge)。線は白帯の高さから床まで。
#   ・アンテナはその線の上に立て、中心が白帯の高さに来るようにする
#     (実物も白帯をまたいで固定され、上に80cm出ている)。
#   ・シグナルの腕は体の前を横切って水平に出し、手のひらは下向き。手は
#     線の上端(白帯)より上、かつ線より右＝ネットの向こう側にある。
#   ・アンテナと縦線は腕より後に描く。先に描くと腕が前に来てアンテナを
#     巻き込んで見えなくしてしまう。
# 肘の点(関節の丸)は描かない。水平な前腕の途中に丸が入ると、肘ではなく
# 「肘掛け付きの椅子」に見えてしまうため。
add("sig21", "オーバーネット", "審判",
    floor(246, 34, 214) +
    rest_arm("right") +
    arm3(76, 92, 102, 98, 148, 94, joint=False) +
    hand("down", 150, 94, 0) +
    net_edge(100, 124, 246) +
    antenna(100, 64, 184),
    "手のひらを下に向け、ネットの上方(相手コート側の空間)にかざす"
    "(白帯に触れるタッチネットと違い、手は白帯より上に出て"
    "ネットの垂直面を越える)")
