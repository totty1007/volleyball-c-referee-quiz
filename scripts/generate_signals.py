...
add_raw("sig21", "オーバーネット",
    svg_wrap_two(
        ...
        # 正面から: 教科書の図と同じ構図にする。ネットを正面(面をこちらに
        # 向けた状態)で画面いっぱいに渡し、端にアンテナを立てて、審判は
        # ネットの向こう側に立つ。白帯が体の高さに来るので「手が白帯より
        # 上にある」ことが一目で読める。
        #  ・下ろした側の腕はネットより先に描く(白帯から下は網に隠れる)
        #  ・シグナルの腕は手前側の肩(124,92)から出す。反対の肩から体の前を
        #    横切らせると、腕の付け根が胸の真ん中に見えてしまう。
        #  ・肘は肩より少し高い位置で曲げ(肘角度およそ120度)、前腕を白帯へ
        #    向けて下ろす。肩から手まで一直線に伸ばすと「差し出す手」に見え、
        #    「かざす手」にならない。
        body("") +
        arm3(76, 94, 68, 116, 66, 140) +
        net_panel_h(126, 4, 226, 70) + antenna(38, 62, 170) +
        arm3(124, 92, 148, 78, 178, 96) + hand("down", 180, 96, 3) +
        gap_arrow(208, 104, 126, w=5.5)),
