...
# 教科書の図(2026-08-28にいただいた写真)は、このシグナルを1枚の絵で描いて
# いる。構図は次のとおりで、v3.7までの「横から見た図＋正面から見た図」の2面図は
# これに置き換えた(1枚で足りるうえ、2面図は他のシグナルより図が小さくなる)。
#   ・審判は正面向きで、ネットの垂直面(赤い破線)が体の中心を縦に貫く
#     ＝審判はネットの面の上に立っている
#   ・ネットは体の右から奥へ遠近法で伸ばし、床の線まで描く
#   ・シグナルの腕は体の前を横切って水平に出し、手のひらは下向き
#   ・手は白帯よりはっきり上にあり、その離隔を両矢印で示す(⑳との違い)
# 肘の点(関節の丸)は描かない。水平な前腕の途中に丸が入ると、肘ではなく
# 「肘掛け付きの椅子」に見えてしまうため。
add("sig21", "オーバーネット", "審判",
    floor(246, 34, 214) +
    rest_arm("right") +
    net_persp(118, 146, 226, 130, 60, 46) +
    net_plane(100, 6, 246) +
    arm3(76, 92, 104, 100, 150, 100, joint=False) +
    hand("down", 152, 100, 0) +
    gap_arrow(196, 112, 136),
